...
func ColorfulJson(str string) string {
	...
	if err := f.Format(buf, []byte(str)); err != nil {
		return str
	}
	return buf.String()
}
...
func ColorFunc(color uint8) SprintfFunc {
	return func(format string, a ...interface{}) string {
		text := fmt.Sprintf(format, a...)
		if color <= 0 {
			return text
		}
		return fmt.Sprintf("%s%s%s", ColorStart(color), text, EndColor)
	}
}

var (
	DefaultSpaceColor       = ColorFunc(0)
	DefaultCommaColor       = ColorFunc(Gray)
	DefaultColonColor       = ColorFunc(Gray)
	DefaultObjectColor      = ColorFunc(Gray)
	DefaultArrayColor       = ColorFunc(Gray)
	DefaultFieldQuoteColor  = ColorFunc(Blue)
	DefaultFieldColor       = ColorFunc(Blue)
	DefaultStringQuoteColor = ColorFunc(Cyan)
	DefaultStringColor      = ColorFunc(Cyan)
	DefaultTrueColor        = ColorFunc(Cyan)
	DefaultFalseColor       = ColorFunc(Cyan)
	DefaultNumberColor      = ColorFunc(Cyan)
	DefaultNullColor        = ColorFunc(Magenta)
...
func newFormatterState(f *Formatter, dst io.Writer) *formatterState {
	sprintfSpace := f.SpaceColor
	sprintfComma := f.CommaColor
	sprintfColon := f.ColonColor
	sprintfObject := f.ObjectColor
	sprintfArray := f.ArrayColor
	sprintfFieldQuote := f.FieldQuoteColor
	sprintfField := f.FieldColor
	sprintfStringQuote := f.StringQuoteColor
	sprintfString := f.StringColor
	sprintfTrue := f.TrueColor
	sprintfFalse := f.FalseColor
	sprintfNumber := f.NumberColor
	sprintfNull := f.NullColor
